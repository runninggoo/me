      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
          ...
          <div class="flex items-center space-x-3">
            ...
            >
              {{ currentRoadmap?.status === 'published' ? '更新发布' : '发布路线图' }}
            </button>
          </div>
        </div>
      </div>
...
const currentRoadmap = computed(() => contentStore.currentRoadmap)

const canPublish = computed(() => {
  return roadmapTitle.value.trim() && 
         roadmapContent.value && 
         roadmapContent.value.length > 0
})
...
const saveDraft = async () => {
  await saveRoadmap('draft')
}

const publishRoadmap = async () => {
  await saveRoadmap('published')
}

const saveRoadmap = async (status = null) => {
  if (!roadmapTitle.value.trim()) {
    alert('请输入路线图标题')
    return
  }
  ...
  const roadmapData = {
    ...
    content: roadmapContent.value,
    status: status || roadmapStatus.value,
    tags: selectedTags.value.map(tag => tag.id)
  }

  if (isEditing.value) {
    roadmapData.id = parseInt(route.params.id)
  }

  const result = await contentStore.saveRoadmap(roadmapData)
  ...
  if (result.success) {
    ...
    if (!isEditing.value && result.data.id) {
      router.replace(`/roadmap/${result.data.id}/edit`)
    }
  } else {
...
const loadRoadmap = async () => {
  if (isEditing.value) {
    const result = await contentStore.loadRoadmap(parseInt(route.params.id))
    if (result.success) {
      const roadmap = result.data
      roadmapTitle.value = roadmap.title
      roadmapDescription.value = roadmap.description || ''
      roadmapContent.value = roadmap.content || []
      roadmapStatus.value = roadmap.status
      selectedTags.value = roadmap.tags || []
    } else {
      alert('加载路线图失败：' + result.error)
      router.push('/dashboard')
    }
  } else {
    // 新建路线图
    contentStore.createNewRoadmap()
  }
